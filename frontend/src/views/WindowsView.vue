<template>
  <div class="flex h-full">
      ...
      <div class="py-3 m-2 h-12 flex items-center justify-between">
        <div>
          ...
          <div class="inline p-3">
            ...
            <label class="font-bold text-base" for="status" v-else>
              Closed
            </label>
          </div>
        </div>
    ...
    <div class="m-auto mt-1 max-w-5xl">
      <h1 class="text-center text-5xl m-7">Windows</h1>
      <WindowsList ref="windowsList" />
    </div>
  </div>
</template>
    ...
    async addWindow() {
      let response = await axios.post(
        "/api/windows/",
        {
          name: this.name,
          roomId: this.room,
          windowStatus: this.status ? "OPEN" : "CLOSED",
        },
        {
          auth: {
            username: "user",
      ...
      this.$refs.windowsList.addWindow(newWindow);
    },
  },
};
</script>
